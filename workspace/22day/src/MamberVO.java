...
// Bean : 데이터를 저장할 맴버 변수(필드)와 멤버 변수에 데이터를 입출력할 수 있는 getter & setter 메소드로만 구성된 클래스를 말한다.
public class MamberVO {
	
//	멤버 변수(필드)를 선언한다.
//	멤버 변수를 선언하고 초기화 시키지 않으면 숫자는 0, 문자는 공백, boolean은 false,클래스로 만든 모든 멤버는 null로 자동 초기화 된다.
//	멤버 변수에 저장된 데이터를 클래스 외부에서 함부로 수정할 수 없도록 일반적ㅇ로 멤버 변수의 접그 권한은 private으로 지정한다.
